  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isSubject) {
      ...
      out.push({
        ...
        explanation: `${noun} は${batchim ? '받침(パッチム)があり' : '母音で終わり'}、主格助詞 ${answer} を使います。`,
        hint: '主語の位置です。받침(パッチム)があれば 이、なければ 가 です。',
      ...
      out.push({
        ...
        options: ['을', '를', '이', '가'],
        correct_answer: answer,
        explanation: `${noun} は${batchim ? '받침(パッチム)があり' : '母音で終わり'}、目的格助詞 ${answer} を使います。`,
        hint: '目的語の位置です。받침(パッチム)があれば 을、なければ 를 です。',
...
async function genVocabMeaning(count: number, level: number, category: string): Promise<Candidate[]> {
  ...
  for (const target of targets) {
    ...
    out.push({
      ...
      explanation: `${target.word} は「${target.meaning}」という意味です。`,
      hint: '単語が使われる場面や漢字・形態素を思い浮かべてみましょう。',
      grammar_tags: [],
